...
import { ShoppingBag, ChevronDown, Star, X, ZoomIn } from "lucide-react";
...
const SIZE_GUIDE = [
  ...
  { size: "XXL", chest: "106-114", waist: "90-98" },
];
    ...
    <div className="min-h-screen pt-28 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2 text-[#888888] text-xs tracking-wide mb-8">
          ...
          <span>/</span>
          <Link href={`/catalog?category=${product.category}`} className="hover:text-white transition-colors">{product.category}</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          <div className="space-y-3">
            <div className="relative aspect-[4/5] bg-[#1a1a1a] rounded-sm overflow-hidden group">
              ...
              <div className="flex gap-2">
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`relative w-16 h-20 overflow-hidden rounded-sm border transition-colors ${
                      selectedImage === i ? "border-white" : "border-[#2e2e2e] hover:border-[#888888]"
                    }`}
                  >
          ...
          <div className="lg:pt-4">
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={12} className={i < Math.round(avgRating) ? "fill-white text-white" : "text-[#2e2e2e]"} />
                  ))}
                </div>
                <span className="text-[#888888] text-xs">({product.reviews.length} reviews)</span>
              </div>
            )}

            <p className="text-white text-2xl font-medium mb-6">{formatPrice(product.price)}</p>

            {/* COD Badge */}
            <div className="inline-flex items-center gap-2 bg-[#1a1a1a] border border-[#2e2e2e] px-3 py-2 rounded-sm text-xs text-[#888888] mb-8">
              💵 Cash on Delivery available · <span className="text-white">+₹100</span>
            </div>
            ...
            {/* Shipping info */}
            <div className="border border-[#2e2e2e] rounded-sm p-4 space-y-2 mb-8">
              <div className="flex items-center gap-3 text-xs text-[#888888]">
                <span className="text-white">🚚</span> Free shipping on orders above ₹999
              </div>
              <div className="flex items-center gap-3 text-xs text-[#888888]">
                <span className="text-white">📦</span> Estimated delivery: 5-7 business days
              </div>
              <div className="flex items-center gap-3 text-xs text-[#888888]">
                <span className="text-white">↩️</span> Easy returns within 7 days
              </div>
            </div>
            ...
            {[
              ...
              { key: "care", title: "CARE INSTRUCTIONS", content: "Machine wash cold at 30°C. Do not tumble dry. Do not iron directly on print. Do not bleach. Wash inside out to preserve colour." },
              { key: "fabric", title: "FABRIC & FIT", content: "Premium performance fabric blend. 88% Polyester, 12% Elastane. Four-way stretch. Quick-dry moisture wicking technology. Pre-shrunk for consistent sizing." },
              ...
              <div key={key} className="border-t border-[#2e2e2e]">
                ...
                >
                  <span className="text-white text-xs tracking-[0.2em] uppercase" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>{title}</span>
                  <ChevronDown
                    size={16}
        ...
        {/* Reviews */}
        {product.reviews && product.reviews.length > 0 && (
          <div className="mt-24 border-t border-[#2e2e2e] pt-12">
            <h2
              style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(2rem, 5vw, 4rem)", lineHeight: 1 }}
              className="text-white tracking-wide mb-8"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {product.reviews.map((review) => (
                <div key={review.id} className="bg-[#1a1a1a] rounded-sm p-6">
                  <div className="flex gap-0.5 mb-3">
                    {Array.from({ length: review.rating }).map((_, i) => (
                      <Star key={i} size={12} className="fill-white text-white" />
                    ))}
                  </div>
                  <p className="text-white text-sm leading-relaxed mb-4">&ldquo;{review.comment}&rdquo;</p>
                  <p className="text-[#888888] text-xs">{review.user.name || "Anonymous"}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <AnimatePresence>
        {sizeGuideOpen && (
          <>
            ...
            />
            <motion.div
              className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0a0a0a] border border-[#2e2e2e] rounded-sm p-8 z-50 w-full max-w-md mx-4"
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
            >
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-white tracking-widest" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.5rem" }}>
                  SIZE GUIDE
                </h2>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {SIZE_GUIDE.map((row) => (
                    <tr key={row.size} className="border-b border-[#1a1a1a]">
                      <td className={`py-3 font-medium ${selectedSize === row.size ? "text-white" : "text-[#888888]"}`}>{row.size}</td>
              ...
              <p className="text-[#888888] text-xs mt-6">All measurements are in centimetres. If between sizes, size up.</p>
            </motion.div>
          </>
        )}
      </AnimatePresence>
